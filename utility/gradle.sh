...
if [[ ${WORKING_DIRECTORY} != ${PROJECT_DIRECTORY}* ]]; then
    echo "Gradle must be executed from within the gainmaster-body-measurement-service project folder"
    exit 1
fi
...
if [ $(docker ps -a | grep gainmaster-body-measurement-service-data-container | wc -l) -ne 1 ] ; then
    docker run $DOCKER_RUN_OPTIONS \
        --name gainmaster-body-measurement-service-data-container \
        --volume /root \
        gainmaster/gradle echo "Data container started"
fi

docker run $DOCKER_RUN_OPTIONS --rm \
    --volumes-from "gainmaster-body-measurement-service-data-container" \
